...
/**
 * 借助此对象访问用户身份信息,权限信息并对 其进行封装,返回给调用者. 思考? 1)此对象由谁来调用?(认证管理器,授权管理器)
 * 
 * @author adminitartor
 */
...
public class ShiroMobileRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		System.out.println("realm.phone");
		// 1.获取用户身份信息
		DefindToken uToken = (DefindToken) token;
		String username = uToken.getUsername();
		// 2.基于用户身份查询数据库信息
		SysUser sysUser = sysUserDao.findUserByUserPhone(username);
		//把查出来的信息放在session中,供在service层获取判断用户启用,禁用状态
